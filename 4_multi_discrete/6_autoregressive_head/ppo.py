...
class PPO(nn.Module):
    def __init__(self):
        ...
        self.fc2   = nn.Linear(77, 256)

        self.action_embedding = nn.Embedding(3, 256)

        self.fc_v  = nn.Linear(256, 1)
        self.optimizer = optim.Adam(self.parameters(), lr=learning_rate)

    def pi1(self, x, softmax_dim = 0):
        out1 = F.relu(self.fc1(x))
        out_pi1 = self.fc_pi1(out1)
        prob1 = F.softmax(out_pi1, dim=softmax_dim)
        return prob1

    def pi2(self, x, a, dim=0):
        out1 = F.relu(self.fc2(x))

        action_emb = self.action_embedding(a)

        #concat1 = torch.cat((out1, torch.squeeze(action_emb)), dim)
        #add1 = out1 + torch.squeeze(action_emb)
        multiply1 = torch.squeeze(action_emb) * out1
        
        out_pi2 = self.fc_pi2(multiply1)
        prob2 = F.softmax(out_pi2, dim=dim)
        return prob2
    ...
    def train_net(self):
        ...
        for i in range(K_epoch):
            ...
            pi1 = self.pi1(s, softmax_dim=1)
            sample_m1 = Categorical(pi1)
            sample_a1 = sample_m1.sample()
            sample_a1 = torch.unsqueeze(sample_a1, 1)

            pi2 = self.pi2(s, sample_a1, dim=1)
    ...
    for n_epi in range(10000):
        ...
        while not done:
            for t in range(T_horizon):
                prob1 = model.pi1(torch.from_numpy(s).float())
                m1 = Categorical(prob1)
                a1 = m1.sample().item()
                prob2 = model.pi2(torch.from_numpy(s).float(), torch.tensor([a1]))
